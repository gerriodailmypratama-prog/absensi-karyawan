    ...
    rows.forEach(r => {
        ...
        tr.innerHTML = '<td><input type="date" class="kh-edit-tgl" data-id="'+(r._id||'')+'" value="'+tanggalYMD+'"></td>'+
            ...
            '<td class="col-aksi-kh">'+
                '<button class="btn-link btn-hapus-absen" data-id="'+(r._id||'')+'" data-nama="'+nama+'" data-tipe="'+(r.tipe||'')+'" data-ts="'+isoTs+'" style="color:#dc2626">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Hapus</button>'+
            '</td>';
        tb.appendChild(tr);
    });
...
async function openEditKaryawan(uid){
    try {
        ...
        const setStat = (id, url) => { const el = $(id); if (el) el.textContent = url ? 'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ sudah diupload' : '(belum)'; };
...
/* ===========================================================
   REKAP KEHADIRAN (Hadirr-style) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ date range summary per user
   =========================================================== */
    ...
    if(events.length===0){
        ...
        tbody.innerHTML = events.map(ev=>{
            ...
            return '<tr data-id="'+ev.id+'">'+
                ...
                '<td>'+status+'</td>'+
                '<td><button class="btn-link rd-hapus" data-id="'+ev.id+'" data-nama="'+(nama||'').replace(/"/g,'&quot;')+'" data-tipe="'+(ev.tipe||'')+'" style="color:#dc2626">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Hapus</button></td>'+
            '</tr>';
        }).join('');
        ...
        tbody.querySelectorAll('.rd-hapus').forEach(b=>{
            b.onclick = async ()=>{
                ...
                try{
                    ...
                    b.disabled = false; b.textContent = 'ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¯ÃÂÃÂ¸ÃÂÃÂ Hapus';
                }
            };
        });
    }
...
// ============================================================
// PAYROLL MODULE ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ gaji harian, dibayar bulanan
// ============================================================
...
// Tujuan: transparan ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ karyawan bisa lihat rincian per hari (full/partial/lembur).
